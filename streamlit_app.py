...
def generate_markdown_report(report: dict, filename: str) -> str:
    md_str = f"# 📄 {filename} - AI 自动审查诊断报告\n\n"
    
    risks = report.get("合规性风险", [])
    if risks:
        md_str += "## 🔴 合规风险 (必须整改)\n\n"
        for i, r in enumerate(risks, 1):
            md_str += f"### ⚠️ 风险 {i}: {r.get('描述', '未知')}\n"
            md_str += f"> **⚡ AI 修复建议**：{r.get('建议', '无')}\n\n"
            
    logics = report.get("逻辑错误", [])
    if logics:
        md_str += "## 🟡 逻辑异常 (自相矛盾)\n\n"
        for i, l in enumerate(logics, 1):
            md_str += f"### 📌 异常 {i}: {l.get('描述', '未知')}\n"
            md_str += f"> **⚡ AI 修复建议**：{l.get('建议', '无')}\n\n"
            
    infos = report.get("核心信息", [])
    if infos:
        md_str += "## 🔵 核心提取信息\n\n"
        for i, info in enumerate(infos, 1):
            md_str += f"- **{info.get('描述', '未知')}**：{info.get('建议', '')}\n"
        ...
        with st.status(">> 正在启动 AI 审查引擎...", expanded=True) as status:
            ...
            if not extracted_text:
                ...
                if review_result.get("status") == "error":
                    ...
                    md_text = generate_markdown_report(report, uploaded_file.name)
                    st.download_button(
                        label="📥 下载 Markdown 格式报告文件",
                        data=md_text,
                        file_name=f"【AI特检】{uploaded_file.name}.md",
                        mime="text/markdown",
